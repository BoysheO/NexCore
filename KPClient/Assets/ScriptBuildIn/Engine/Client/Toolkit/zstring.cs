...
/*
 BoysheO：原版有一些小bug，这个是修复过的。仍有bug也可联系我
 介绍：
...
namespace GameFramework
{
    ...
    public class zstring
    {
        ...
        internal static zstring_block g_current_block;//zstring所在的block块
        ...
        internal class zstring_block : IDisposable
        {
        ...
        public unsafe static implicit operator zstring(int value)
        {
            ...
            // finally: iterate again, get the char of each digit, memcpy char to result
            bool negative = value < 0;
            value =value == int.MinValue ? int.MaxValue : Math.Abs(value);//avoid  Negating the minimum value of a twos complement number is invalid.
            int num_digits = get_digit_count(value);
            zstring result;
            if (negative)
            {
        ...
        //double->zstring转换
        public static implicit operator zstring(double value)
        {
            return (zstring)(float)value;
        }
