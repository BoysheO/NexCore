...
namespace BoysheO.Toolkit
{
    public sealed class SRandom
    {
        ...
        [Obsolete("GetHashCode may gen different num between 2 platform", true)]
        ...
        {
            //div may not same between platforms
            // return (int) (NextSample() * (1.0D / int.MaxValue) * (maxValue - minValue)) + minValue;
            if (maxValue <= minValue) throw new ArgumentOutOfRangeException(nameof(minValue));
            var len = maxValue - minValue;
            var l = NextSample() % len; //这个算法会导致靠近minValue的值出现概率大一点。但是没办法了只能先这样用
            return minValue + l;
        }
        ...
        public int NextIndex<T>(IList<T> list)
        {
        ...
        public void Choose<T>(IList<T> items, int quantity, IList<T> result)
        {
            //var buffer = new List<T>(quantity);
            result.Clear();
            for (var i = 0; i < quantity; i++) result.Add(Choose(items));
        }
        ...
        {
            if (saveState.Length != 59) throw new Exception("Random state was corrupted!");
