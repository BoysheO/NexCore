...
namespace BoysheO.UnityEditor
{
    public static class CommandHelper
    {
            ...
            bool printLog = true, CancellationToken token = default)
        {
            ...
            Debug.Log($"{head}exec:{fileName} {argument}");
            ...
            if (printLog)
            {
                Debug.Log($"<color=red>{head}-------------Start read standard output--------------</color>");
            }
            ...
            var t1 = Task.Run(async () =>
            {
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    if (printLog) Debug.Log("[Normal]" + head + line);
                }
            }, tk3.Token);
            ...
            var t2 = Task.Run(async () =>
            {
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    if (printLog) Debug.LogError("[Error]" + head + line);
                }
            }, tk3.Token);
